    ...
    <form method='post' action="{{ route('products.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                ...
                <input type="text" name="name" class="form-control  @error('name') is-invalid @enderror" id="inputEmail4d" value="{{old('name')}}">
            </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-4">
                ...
                <input type="number" name="price" class="form-control @error('price') is-invalid @enderror"
                    id="inputAddress" placeholder="1234 Main St" value="{{old('price')}}">
                @error('price')
                    <div class="alert alert-danger"> {{ $message }} </div>
                @enderror
            </div>
            ...
            <div class="form-group col-md-4">
                ...
                <input type="number" name="quantity" class="form-control @error('quantity') is-invalid @enderror"
                    id="inputAddress2" placeholder="Apartment, studio, or floor" value="{{old('quantity')}}">
                @error('quantity')
                    <div class="alert alert-danger"> {{ $message }} </div>
                @enderror
            </div>
            ...
            <div class="form-group col-md-4">
                ...
                <select name="subcategory_id" id="inputStateq"
                    ...
                    @foreach ($subCategories as $key => $subcat)
                        <option {{old('subcategory_id') == $subcat->id ? 'selected' : ''}} value="{{ $subcat->id }}">{{ $subcat->name }}</option>
                    @endforeach
                </select>
